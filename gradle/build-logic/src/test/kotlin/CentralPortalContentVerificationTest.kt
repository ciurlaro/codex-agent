...
class CentralPortalContentVerificationTest {
    ...
    fun `tampered or missing remote file cannot establish proof`() {
        listOf(
            downloadResponse(CENTRAL_ANDROID_AAR_ENTRY, CentralPortalResponse(200, "tampered")),
            downloadResponse(CENTRAL_ANDROID_AAR_ENTRY, CentralPortalResponse(404, "missing")),
    ...
    fun `same-coordinate tampering after substring collision is rejected by file bytes`() = withCentralFixture { fixture ->
        val portal = FakePortal(
            ...
            status("VALIDATED", name = fixture.name),
            downloadResponse(CENTRAL_ANDROID_AAR_ENTRY, CentralPortalResponse(200, "wrong bytes")),
        )
    ...
    @Test
    fun `remote verification is bounded to four workers and returns deterministic order`() {
        val active = AtomicInteger()
        val maximum = AtomicInteger()
        val barrier = CyclicBarrier(4)

        val result = parallelCentralMap((0 until 8).toList()) { value ->
        ...
        assertEquals((0 until 8).map { it * 2 }, result)
    }

    @Test
    fun `parallel failure waits for all sender activity`() {
        val barrier = CyclicBarrier(4)
        val failed = CountDownLatch(1)
        val completed = AtomicInteger()

        val failure = assertFailsWith<IllegalStateException> {
            parallelCentralMap((0 until 4).toList()) { value ->
                barrier.await()
                if (value == 0) {
                    failed.countDown()
                    error("first failure")
                ...
                failed.await()
                val deadline = System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(50)
