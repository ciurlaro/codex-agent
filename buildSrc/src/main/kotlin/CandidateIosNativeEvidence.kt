...
internal fun verifyCandidateIosNativeEvidence(file: File, expectedCommit: String) {
    ...
    check(evidence.keys == setOf(
        ...
        "rustSrcComponent", "xcodeVersionSha256", "swiftVersionSha256", "nativeTestsProofSha256", "slices",
    )) { "iOS native evidence schema fields are invalid" }
    check(evidence.releaseInt("schemaVersion") == 1 &&
        evidence.releaseString("protocol") == "codex-agent-ios-native-evidence-v1") {
    ...
    val byTarget = slices.associateBy { slice ->
        check(slice.keys == setOf("target", "archive", "proofSha256")) { "iOS native slice fields are invalid" }
