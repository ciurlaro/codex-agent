...
class AppleRustSliceEvidenceTest {
    ...
    fun `test command and identity mismatches are rejected`() = fixture().use {
        assertFailsWith<IllegalStateException> {
            verifyAppleRustEvidenceDirectory(
                it.evidence,
                it.identity.copy(compilerSettingsSha256 = "f".repeat(64)),
                it.commands,
            )
        }
        assertFailsWith<IllegalStateException> {
    ...
    fun `evidence tasks always execute live checkout validation`() {
        ...
        assertEquals(2, Regex("init \\{ outputs\\.upToDateWhen \\{ false } }").findAll(source).count())
    }
}
...
private class EvidenceFixture : AutoCloseable {
    ...
    private val settings = mapOf("rustToolchain" to "1.95.0", "rustSrcComponent" to "required", "releaseLto" to "fat")
    val identity = AppleRustEvidenceIdentity(
    ...
    init {
        appleRustSliceSpecs.forEach { spec ->
            ...
            evidence.resolve(spec.proofName).atomicWriteJson(buildAppleRustSliceProof(spec, archive, identity))
        }
        evidence.resolve(IOS_NATIVE_TESTS_PROOF).atomicWriteJson(buildAppleNativeTestsProof(identity, commands))
    }

    fun verify() = verifyAppleRustEvidenceDirectory(evidence, identity, commands)
